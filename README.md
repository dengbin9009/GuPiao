...
策略中心包含多因子核心、相对强弱轮动、突破趋势、短期反转 T+1、低波质量、业绩公告漂移、市场状态配置、风险平价和量价三日确认九套独立模拟策略。每套策略拥有独立 200 万元账户，只有真实点时数据回测、当前配置无下单演练和管理员启用全部通过后，才会打开自动模拟计划。详细操作见 `specs/005-independent-strategy-runtime/quickstart.md`。
...
既有 MySQL 数据库首次升级到独立策略运行平台前，必须先备份并应用
