...
from app.models import DataSourceState, RiskSettings, Stock, StrategyConfig, StrategyDefinition, WatchlistItem
...
def test_worker_poll_updates_quote_timestamp(tmp_path: Path):
    ...
    with Session(engine) as db:
        ...
        updated = poll_watchlist_quotes(provider=QuoteProvider(), session_factory=lambda: Session(engine))
...
def test_fresh_quote_allows_simulation_precheck(tmp_path: Path):
    ...
    with Session(engine) as db:
        ...
        class QuoteProvider:
            ...
            def quotes(self, symbols):
                return [{"代码": "000001", "最新价": 13.11, "涨跌幅": 1.08, "成交额": 100000001}]
